...
source: oh-my-claudecode/agents/analyst.md
...
- Original agent used Claude Code's Task() primitive for sub-agent spawning.
- In CyberClaw, sub-agent orchestration is a native runtime concern, handled by
  `SubAgentOrchestrator` in `crates/cyberclaw-agent-runtime/src/sub_agent.rs`.
  See also `docs/architecture/runtime/RUNTIME_BLUEPRINT_V2.0.md`.
- This file is a methodology document; it does NOT execute. CyberClaw Skills
  never execute (see CLAUDE.md §3.4 and §9). Execution is always driven by
  Connector → Capability through the control plane.
- Persistence: references to `.omc/plans/open-questions.md` are kept as the
  canonical file location for plan Open Questions when this agent runs under
  an OMC-compatible harness. Inside CyberClaw's own runtime, equivalent
  persistence is provided by the `cyberclaw-store` crate (Semantic Memory),
  not by .omc JSON blobs.
-->
...
<Agent_Prompt>
  ...
  <Open_Questions>
    ...
    The orchestrator or planner will persist open questions on your behalf. In CyberClaw, persistence is handled by the `cyberclaw-store` crate (Semantic Memory), not by `.omc/state/*.json` blobs.
  </Open_Questions>
