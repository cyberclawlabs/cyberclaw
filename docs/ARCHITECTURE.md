...
| **Sandbox / isolation** | The same Capability can run under multiple runtimes: local, isolated process, container, or remote. Container runtimes support three named profiles — `minimal` (no network, read-only filesystem), `dev` (workspace writes, no network), `isolated` (full lockdown) — selected per-capability. High-risk operations default to container isolation. A failure or compromise in one Agent does not contaminate another. |
...
Two complementary mechanisms keep an Agent's iteration loop predictable.

**Loop budgets.** Every Agent session runs under a budget envelope — wall-clock time, accumulated tokens, and a repetition-detection window. Three named profiles (`L1` / `L2` / `L3`) cover typical use cases from short interactive turns to long autopilot runs. The loop is forced to stop before it hits model limits or accumulates identical-call cycles.

**Output verifiers.** Capability outputs can be checked structurally before the Agent acts on them — JSON-shape assertions, regex matches, presence of fenced code blocks. Failed verification surfaces the mismatch as a tool-result error, letting the model retry with corrected inputs instead of acting on malformed data.

Both mechanisms run inside the same dispatch path described above — they never bypass governance.
